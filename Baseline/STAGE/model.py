...
def STAGE(
    ...
    if experiment=='generation' and data_type=='10x':
        ...
        used_gene, normed_data, adata_sample = get_data(adata, experiment=experiment, sample_index=sample_index, sample_barcode=sample_barcode, path1=path1)    #返回采样后的基因
    ...
    if experiment == 'recovery':
        ...
        normed_coor_df.to_csv(path1+"/coord.txt", header=0, index=0)        #保存下采样后并归一化的坐标（作为训练数据）
        normed_fill_coor_df.to_csv(path1+"/fill_coord.txt", header=0, index=0)      #保存所有归一化的坐标
        X_dim = 2
