...
class ConvNetwork(nn.Module):
    def __init__(self, patch_size=16, in_c=1, embed_dim=64, depth=5):
        super(ConvNetwork, self).__init__()

        # 设置卷积层，确保stride=1，避免尺寸压缩
        self.conv1 = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=1, padding=patch_size // 2)
        self.relu = nn.ReLU()

        # 设置一系列卷积层
        self.conv_layers = nn.ModuleList()
        for _ in range(depth):
            self.conv_layers.append(nn.Conv2d(embed_dim, embed_dim, kernel_size=3, stride=1, padding=1))
            self.conv_layers.append(nn.ReLU())

        # 最后一个卷积层输出4个通道
        self.final_conv = nn.Conv2d(embed_dim, 4, kernel_size=1)

    def forward(self, x):
        # 输入数据通过第一次卷积处理
        x = self.conv1(x)  # 输入 shape: (b, 1, h, w) -> (b, embed_dim, h, w)
        x = self.relu(x)
        ...
        # 使用 F.interpolate 进行精确上采样
        x = F.interpolate(x, size=(40, 36), mode='bilinear', align_corners=False)
