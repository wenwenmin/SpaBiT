...
train_adata = sc.read_visium(path="data\\DLPFC\\151507", count_file='filtered_feature_bc_matrix.h5')
...
train_adata = train_adata[:, train_adata.var["n_cells_by_counts"] > len(train_adata.obs.index)*0.1] #保留表达量较高的基因（被 >10% cell 表达）
train_counts = np.array(train_adata.X.todense())
train_coords = train_adata.obs[['array_row', 'array_col']]

train_lr, train_hr, in_tissue_matrix = get10Xtrainset(train_counts, train_coords)   #得到一个下采样后的规整网格和原始网格
...
#
truth_counts, SpaViT_data, interpolation_data = process_data.get_10X_DLPFC(151507)
#truth_counts, [simu_3D, impute_position, simu_coords, adata_var_names], [simu_counts, impute_position,simu_coords, adata_var_names]
